...
def checkRemovableDrives() -> dict[str, str]:
    ...
    for letter in string.ascii_uppercase[2:]:
        ...
        if (
            bitmask & 1
            and ctypes.windll.kernel32.GetDriveTypeA(drive.encode("ascii"))
            == DRIVE_REMOVABLE
        ):
            volume_name = ""
            name_buffer = ctypes.create_unicode_buffer(1024)
            filesystem_buffer = ctypes.create_unicode_buffer(1024)
            error = ctypes.windll.kernel32.GetVolumeInformationW(
            ...
            if (
                ctypes.windll.kernel32.GetDiskFreeSpaceExA(
                    drive.encode("ascii"), ctypes.byref(free_bytes), None, None
                )
                == 0
            ):
                continue
...
class RemoveUSB(FlowLauncher):
    ...
    def performEjectDevice(self, device: str):
        # Magic WinAPI stuff
        ...
        handle = ctypes.windll.kernel32.CreateFileA(
            "\\\\.\\{0}".format(device[:-1]).encode("ascii"),
        ...
        return_code = ctypes.windll.kernel32.DeviceIoControl(
            handle,
